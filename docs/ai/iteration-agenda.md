...
### Iteration 30 — Reusable technical evidence contract and prompt-context boundary (implemented)
...
The implementation provides a source-neutral `zeus.technical-evidence-context`
contract, a strict local CLI projection, and a programmatic API. The projection
accepts only a closed anonymized technical-evidence envelope, rejects raw or
unsupported fields, preserves uncertainty and warning codes, applies deterministic
node/edge/token bounds, and emits only generic technical relationships plus
opaque identifiers. Synthetic tests cover deterministic reruns, unsafe-boundary
rejection, bounded omissions, CLI path safety, and contract registration.

The public repository contains the contract, synthetic fixtures, structural
guards, and generated catalog metadata only. Source-derived artifacts, runtime
metrics, fingerprints, warning instances, and local references remain local and
are not part of documentation, CI artifacts, commits, or releases. No process
catalog or business interpretation is generated by this iteration.

### Iteration 31 — Evidence-aware prompt adapters and local review receipts (implemented)

Build the next reusable Promptkit layer on top of the technical evidence
context contract:

- render a deterministic, bounded prompt envelope from approved technical
  tokens, opaque identifiers, relationship kinds, and explicit uncertainty;
- reject free-form or unsupported fields before prompt construction;
- expose the adapter through the CLI and programmatic API while keeping normal
  source-oriented prompt templates separate;
- bind local review receipts to the exact context and prompt fingerprints;
- keep reviewer identity hashed and keep promotion, deployment, and external
  publication disabled;
- cover deterministic output, fail-closed validation, exact receipt binding,
  CLI path safety, and synthetic privacy/credential canaries.

The implementation provides `zeus.technical-evidence-prompt` and
`zeus.technical-evidence-review` contracts. The CLI routes are
`technical-evidence prompt`, `technical-evidence review`, and
`technical-evidence review-check`. Outputs contain only bounded technical
