...
- For confidential local legacy sources, run `legacy-source inventory` first and keep the source root read-only; emit only anonymized counts, evidence IDs, fingerprints, and warning codes under `.local/legacy-source-inventory/`. The next technical-only step is `legacy-source graph`, which emits only HMAC IDs and generic evidence edges.
...
- When the same sanitized candidate appears in separate catalogs, write their review reports to local `.zeus/` files and run `node cli/zeus.js process promotion-check --candidate <.zeus/report-a.json> --candidate <.zeus/report-b.json> --fixture <.zeus/process-regression-fixture.json> --json`; only cross-catalog signals with a valid sanitized fixture become `ready-for-human-review`, never automatically promoted.
- When a catalog, prompt, glossary, or retrieval rule changes, run `node cli/zeus.js process regression-check --catalog <catalog> --corpus <sanitized-corpus> --json`, obtain the anonymous catalog fingerprint and evaluation ID, have a domain owner create a matching `.zeus` reviewer decision, and rerun with `--decision`; this gate is read-only and never auto-promotes.
- After a new regression result exists, compare it to the last known-good `.zeus` result with `node cli/zeus.js process drift-check --baseline .zeus/process-answer-regression-baseline.json --current .zeus/process-answer-regression.json --out .zeus/process-answer-drift.json --json`; inspect stable drift codes and never treat the report as automatic approval.
- For reviewer-friendly explanations, run `node cli/zeus.js process drift-review --drift .zeus/process-answer-drift.json --history .zeus/process-answer-review-history.json --out .zeus/process-answer-review.json --json`; the optional history is exact-drift, sanitized, and hashed at output, and an approved review never promotes knowledge automatically. To find unresolved conflicts across multiple drift identities, run `node cli/zeus.js process drift-review-summary --history .zeus/process-answer-review-history.json --out .zeus/process-answer-review-summary.json --json` and resolve every reported finding before relying on a latest decision. To classify review evidence and preview superseded retention candidates, run `node cli/zeus.js process drift-review-retention --history .zeus/process-answer-review-history.json --as-of <ISO-timestamp> --json`; this is read-only and never deletes history automatically. When the triage must be retained for later human review, run `node cli/zeus.js process drift-review-receipt --history .zeus/process-answer-review-history.json --as-of <ISO-timestamp> --out .zeus/process-answer-review-receipt.json --json`; the receipt records its inspection basis and history fingerprint, never an approval, deletion, or promotion.
...
- Confidential legacy source available: read [`docs/ai/legacy-source-boundary.md`](legacy-source-boundary.md), then run `legacy-source inventory --source-root <approved-local-root> --out .local/legacy-source-inventory/inventory.json --json` before any analysis or process discovery. After a safe inventory, use `legacy-source graph --source-root <approved-local-root> --inventory .local/legacy-source-inventory/inventory.json --out .local/legacy-source-inventory/evidence-graph.json --json` only for technical evidence linkage. Never publish process knowledge automatically from either artifact.
...
- Business-process question: use `process list|describe|query|chat|view|impact|diff|evaluate` with an explicit process catalog and, when needed, a scoped glossary; preserve evidence, freshness, confidence, and unknowns in the answer. Use `chat` for the local conversational adapter, `view --role product-owner|architect|developer|tester` for a bounded audience projection, and `evaluate` before relying on a catalog broadly. For regression changes, use `regression-check`, `drift-check`, the reviewer-only `drift-review` projection, `drift-review-summary` for a bounded multi-drift history scan, and `drift-review-retention` for deterministic freshness/retention triage.
