...
| `technical-evidence` | — | `context`, `prompt`, `review`, `review-check` | `stable` | `S1` | Local anonymized evidence | local-read, local-artifact-write | cli | — | Project already-anonymized technical evidence into a bounded source-neutral prompt and record/check local review receipts. The boundary rejects raw content, paths, names, credentials, business terms, and unsupported fields. | `node cli/zeus.js technical-evidence prompt --help` |
