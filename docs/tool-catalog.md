...
| `context` | — | `show`, `set`, `clear` | `stable` | `S1` | Local workspace state | local-context-write | cli, mcp | — | Show or update the credential-free working context for the active system, source library/file/member, metadata schema, and data scope. Explicit command arguments always override it. | `node cli/zeus.js context set --profile dev --active sourceCode --system test --source-library APPLIB --source-file QRPGLESRC --member ORDERPGM --source-root ./rpg_sources` |
...
| `legacy-source` | — | `inventory`, `graph` | `experimental` | `S1` | Local source boundary | local-read, local-artifact-write | cli | — | Read a local legacy-source tree without changing it and emit only anonymized inventory and technical evidence-graph aggregates. Raw paths, source text, credentials, and business terms are never emitted. | `node cli/zeus.js legacy-source inventory --help` |
| `technical-evidence` | — | `context`, `prompt`, `review`, `review-check`, `regression`, `policy-check`, `bundle`, `handoff`, `bundle-check`, `acceptance-check`, `acceptance-history` | `stable` | `S1` | Local anonymized evidence | local-read, local-artifact-write | cli | — | Project already-anonymized technical evidence into a bounded source-neutral prompt, check prompt regressions, enforce a local-only egress gate, create fingerprint-only bundles and local handoff receipts, replay bundles for integrity verification, evaluate a final local acceptance matrix, and retain a bounded local acceptance history. The boundary rejects raw content, paths, names, credentials, business terms, and unsupported fields. | `node cli/zeus.js technical-evidence acceptance-history --help` |
...
| `describe-table` | — | — | `stable` | `S2` | Configured JDBC read | remote-read | cli, mcp | — | Read schema-qualified column metadata from a named external JDBC connection; row counts are opt-in. | `node cli/zeus.js describe-table --profile external-readonly --connection reporting --table APPDATA.ORDERS --json` |
...
| `query-sql` | `sql` | — | `stable` | `S2` | DB2 or configured JDBC read | remote-read | cli, mcp | — | Run one or more read-only SQL statements (SELECT/WITH). Use --connection for a named external JDBC target; DB2 schema/library overrides remain DB2-only. | `node cli/zeus.js query-sql --profile default --sql "SELECT * FROM QSYS2.SYSTABLES FETCH FIRST 10 ROWS ONLY; SELECT CURRENT_USER FROM SYSIBM.SYSDUMMY1"` |
...
| `journal-row-diff` | — | — | `stable` | `S2` | DB2 read | remote-read | cli | — | Compare journal before/after row images with an independent read-only audit query and report aggregate no-op versus content-change counts. Raw journal bytes and row values are never emitted. | `node cli/zeus.js journal-row-diff --profile default --journal-library APPDATA --journal-name APPJRN --layout "ID:P:3:0,STATUS:C:1:0" --key-columns ID --audit-query "SELECT ID, CHANGED_AT FROM APPDATA.AUDIT_LOG" --start 2026-08-21-10.00.00 --end 2026-08-21-11.00.00 --json` |
...
| `test-run` | — | `start`, `capture`, `show`, `rollback` | `stable` | `S2` | DB2 read + local | remote-read, local-artifact-write | cli, mcp | — | Capture before and after test snapshots plus rollback SQL text. | `node cli/zeus.js test-run start --profile default --program ORDERPGM --table APPLIB.APP_TABLE_00 --key ID=1` |
...
| `agent` | — | `bootstrap`, `preflight`, `prompt`, `suggest`, `evaluate`, `evaluate --list`, `feedback`, `feedback review`, `log`, `log list`, `log summary`, `log suggest` | `stable` | `S0` | Local | none | cli | — | Orient CLI agents, generate context-aware prompts, suggest bounded evidence-first workflows, evaluate responses, review contract-improvement candidates against sanitized fixtures and explicit diffs, and record/read local experience events without executing work. | `node cli/zeus.js agent preflight --goal "<goal>" --json` |
| `process` | — | `list`, `describe`, `query`, `chat`, `view`, `impact`, `diff`, `evaluate`, `experience`, `improvements`, `promotion-check`, `regression-check`, `drift-check`, `drift-review`, `drift-review-summary`, `drift-review-retention`, `drift-review-receipt`, `glossary list`, `glossary resolve` | `experimental` | `S1` | Local read-mostly | local-read | cli, api | — | Retrieve evidence-backed business-process projections, role-specific views, a local read-only chat adapter, deterministic quality evaluations, versioned process-answer regression checks with explicit reviewer decisions, bounded drift reports, reviewer-friendly explanations, review-history summaries, freshness/retention previews and reproducible review receipts across drift identities, and project-specific legacy vocabulary from explicit local catalogs. Record sanitized process learnings, combine anonymized signals across catalogs, and produce review-only improvement, promotion-readiness, regression-gate, drift, approval-history, retention, and review-receipt reports without publishing, deleting, or mutating knowledge. Exact identifiers, scoped glossary mappings, and reviewed facts are ranked ahead of derived summaries; ambiguity, freshness, evidence, confidence, and unknowns remain visible. | `node cli/zeus.js process chat --catalog ./output/process-candidates.json --question "Was macht Schnittstelle XY?" --json` |
...
5. `agent log summary --json` (recurring failures and reusable lessons)
6. `agent log suggest --goal "<goal>" --json` (goal-matched sanitized workarounds)
7. `agent feedback --json` (reviewable candidates from repeated lessons and evaluation findings)
8. `agent feedback review --candidate .zeus/agent-feedback.json --before <path> --after <path> --fixture <path> --json` (verify a sanitized regression fixture and explicit contract diff before promotion)
9. `tools list --json` and `tools describe <id> --json` (capability discovery)
10. `context show --json` (current scope and environment)
11. `doctor` (before profile-based remote work)
12. `fetch` (only if source refresh is needed and approved)
13. `spool-read` (only if existing IBM-i spool output is needed; bounded `S2` read)
14. `analyze` or `workflow --preset ...`
15. `query-table`/`query-sql`/`joblog`/`field-search`/`search-source`/`inspect-object` for evidence deepening
16. `impact`/`assess-risk`/`generate-test`/`generate-checklist`/`qa` for planning and validation
17. `agent log --outcome ...` after failed, blocked, partial, or corrected attempts (iterative learning)
18. generated artifacts and `bundle` for review/sharing; optional `serve` for local viewing
19. `upsert`/`upsert-sql`/`insert`/`update` only after explicit user approval
20. `bridge` only in operator-gated, explicitly approved flows
