...
| `technical-evidence` | — | `context` | `stable` | `S1` | Local anonymized evidence | local-read, local-artifact-write | cli | — | Project an already-anonymized technical evidence graph into bounded local prompt context. The boundary rejects source text, paths, names, credentials, business terms, and unsupported fields. | `node cli/zeus.js technical-evidence context --help` |
