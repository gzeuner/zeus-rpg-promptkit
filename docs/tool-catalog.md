...
Last generated: 2026-05-21 13:18:31
-->
...
| `mcp` | `S0` | Local read-only | Start local MCP stdio server for safe read-only Zeus tool exposure with allowlist policy gating and opaque cursor pagination on supported tools. | `node cli/zeus.js mcp serve --verbose --allow-tools zeus.health,zeus.workflow,zeus.bundle,zeus.analyze,zeus.impact,zeus.assess-risk,zeus.query-table,zeus.query-sql,zeus.search-source,zeus.field-search,zeus.joblog,zeus.inspect-object` |
