...
| `legacy-source` | — | `inventory`, `graph` | `experimental` | `S1` | Local source boundary | local-read, local-artifact-write | cli | — | Read a local legacy-source tree without changing it and emit only anonymized inventory and technical evidence-graph aggregates. Raw paths, source text, credentials, and business terms are never emitted. | `node cli/zeus.js legacy-source inventory --source-root <local-root> --out .local/legacy-source-inventory/inventory.json --json` |
