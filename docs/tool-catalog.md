...
| `project-knowledge` | `project-intelligence` | `stable` | `S1` | Local | local-read, local-artifact-write | cli, api, mcp | — | Thin public adapter for Project Intelligence: discover integrated capability presence and dispatch create/index/query/impact/context/inspect/verify only when an entitled built-in module or explicitly supplied external extension is registered. Fails closed when capabilities are absent. | `node cli/zeus.js project-knowledge discover --json && node cli/zeus.js project-knowledge query --knowledge-root ./pk --project-id demo --trusted-roots "[{\"rootId\":\"src\",\"path\":\"./src\"}]" --query ORDERPGM --json` |
