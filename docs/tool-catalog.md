...
Last generated: 2026-08-08T00:00:00.000Z
-->
...
Package: `zeus-rpg-promptkit@0.2.0`
...
| `analyses` | — | `list`, `register`, `index`, `open`, `show`, `unregister` | `stable` | `S1` | Local | local-artifact-write | cli, mcp | — | List, register, inspect, and open locally tracked analysis artifacts. | `node cli/zeus.js analyses list --profile default` |
...
| `knowledge` | — | `extract`, `validate`, `inspect` | `experimental` | `S1` | Local | local-artifact-write, local-read | cli | — | Extract, validate, and inspect privacy-gated project-neutral knowledge catalogs from local PUI structure. | `node cli/zeus.js knowledge extract --mode ui-patterns --file ./display/DSPFILE.MBR --out ./output --run-id run-001` |
...
| `process` | — | `list`, `describe`, `query`, `impact`, `diff`, `glossary list`, `glossary resolve` | `experimental` | `S0` | Local read-only | local-read | cli, api | — | Retrieve evidence-backed business-process projections and resolve project-specific legacy vocabulary from explicit local catalogs. Exact identifiers, scoped glossary mappings, and reviewed facts are ranked ahead of derived summaries; ambiguity, freshness, evidence, confidence, and unknowns remain visible. | `node cli/zeus.js process query --catalog ./output/process-candidates.json --glossary ./output/process-glossary.json --question "Was macht Schnittstelle XY?" --json` |
