...
| `process` | — | `list`, `describe`, `query`, `chat`, `view`, `impact`, `diff`, `evaluate`, `glossary list`, `glossary resolve` | `experimental` | `S0` | Local read-only | local-read | cli, api | — | Retrieve evidence-backed business-process projections, role-specific views, a local read-only chat adapter, deterministic quality evaluations, and project-specific legacy vocabulary from explicit local catalogs. Exact identifiers, scoped glossary mappings, and reviewed facts are ranked ahead of derived summaries; ambiguity, freshness, evidence, confidence, and unknowns remain visible. | `node cli/zeus.js process chat --catalog ./output/process-candidates.json --question "Was macht Schnittstelle XY?" --json` |
