...
| `technical-evidence` | — | `context`, `prompt`, `review`, `review-check`, `regression`, `policy-check`, `bundle`, `handoff` | `stable` | `S1` | Local anonymized evidence | local-read, local-artifact-write | cli | — | Project already-anonymized technical evidence into a bounded source-neutral prompt, check prompt regressions, enforce a local-only egress gate, and create fingerprint-only bundles and local handoff receipts. The boundary rejects raw content, paths, names, credentials, business terms, and unsupported fields. | `node cli/zeus.js technical-evidence bundle --help` |
