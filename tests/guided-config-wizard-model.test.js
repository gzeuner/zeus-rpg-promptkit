...
test('guided configuration payload exposes wizard steps, intents, purpose labels, and discovery actions', () => {
  ...
  assert.ok(
    payload.steps.some(
      step => step.id === 'source-discovery' && Array.isArray(step.fields) && step.fields.length > 0
    )
  );
});
...
test('intent classification marks required, recommended, optional, and not-needed areas', () => {
  ...
  assert.ok(
    classifications.some(
      entry => entry.classification === 'not-needed' || entry.classification === 'optional'
    )
  );
});
...
test('guided configuration draft validation enforces known purpose labels and env-only secret references', () => {
  const valid = validateGuidedConfigurationDraft({
    ...
    sourceLocations: [{ library: 'applib', sourceFile: 'qrpglesrc', purposeLabel: 'rpg-sources' }],
  });
...
test('DB2 discovery preview can stay local-only while deriving honest metadata scope hints', () => {
  ...
  assert.equal(preview.previewKind, 'config-derived-local-preview');
  assert.ok(
    preview.candidates.some(entry => entry.kind === 'metadata-schema' && entry.value === 'METALIB')
  );
  assert.ok(
    preview.candidates.some(
      entry => entry.kind === 'workflow-table' && entry.value === 'APP.ORDERS'
    )
  );
...
test('object discovery preview can stay local-only while deriving bounded library and member hints', () => {
  ...
  assert.equal(preview.previewKind, 'config-derived-local-preview');
  assert.ok(
    preview.candidates.some(entry => entry.kind === 'object-library' && entry.value === 'APPLIB')
  );
  assert.ok(
    preview.candidates.some(entry => entry.kind === 'workflow-member' && entry.value === 'CUSTSRV')
  );
