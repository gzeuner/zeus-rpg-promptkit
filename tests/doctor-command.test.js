...
const {
  ...
  runDoctorChecks,
} = require('../src/cli/commands/doctorCommand');
...
test('runDoctorChecks records probe matrix entries when --probe is enabled', () => {
  ...
  assert.ok(result.probeRows.some((entry) => entry.functionName === 'fetch' && entry.status === 'OK'));
  assert.ok(result.probeRows.some((entry) => entry.functionName === 'metadata-db' && entry.status === 'OK'));
  fs.rmSync(tempRoot, { recursive: true, force: true });
});
