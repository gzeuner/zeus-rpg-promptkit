...
test('V1 smoke flow generates analysis artifacts and bundle outputs', () => {
  ...
  try {
    ...
    for (const fileName of expectedFiles) {
      assert.equal(
        fs.existsSync(path.join(programOutputDir, fileName)),
        true,
        `missing ${fileName}`
      );
    }
    ...
    assert.deepEqual(context.sql.statements.map(statement => statement.tables.join(',')).sort(), [
      'CUSTOMER,ORDERS',
      'INVOICE',
    ]);
    ...
    const prompt = fs.readFileSync(
      path.join(programOutputDir, 'ai_prompt_documentation.md'),
      'utf8'
    );
    ...
    const architectureViewer = fs.readFileSync(
      path.join(programOutputDir, 'architecture.html'),
      'utf8'
    );
    ...
    assert.equal(analyzeManifest.summary.stageCount, 9);
    ...
    assert.ok(analyzeManifest.stages.some(stage => stage.id === 'load-known-facts'));
    ...
    assert.ok(
      analyzeManifest.stages.every(stage =>
        Object.prototype.hasOwnProperty.call(stage, 'definition')
      )
    );
    ...
    assert.ok(
      manifest.artifacts.some(artifact => artifact.path === 'context.json' && artifact.sha256)
    );
