...
test('catalog model is deterministic and preserves the public command contracts', () => {
  ...
  assert.equal(first.generatedAt, '2026-09-20T00:00:00.000Z');
  ...
  for (const command of ['validate-rpg-sql', 'onboarding', 'trace', 'xref', 'display-ui-inspect']) {
    assert.ok(
...
test('generation works in an exported tree without Git metadata', () => {
  ...
  try {
    ...
    assert.equal(model.generatedAt, '2026-09-20T00:00:00.000Z');
