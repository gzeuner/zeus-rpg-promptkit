...
test('guided analyze mode writes task index, selected mode metadata, and mode-specific prompts', () => {
  ...
  try {
    ...
    assert.equal(
      fs.existsSync(path.join(programOutputDir, 'ai_prompt_architecture_review.md')),
      true
    );
    ...
    assert.ok(
      analysisIndex.guidedModes.some(
        entry => entry.name === 'modernization' && entry.selected === true
      )
    );
    ...
    assert.ok(
      modernizationTask.prompts.some(
        entry => entry.name === 'modernization' && entry.generated === true
      )
    );
    assert.ok(Array.isArray(modernizationTask.reviewWorkflow.intendedAudience));
    assert.match(
      modernizationTask.reviewWorkflow.intendedAudience.join('\n'),
      /Modernization leads/
    );
    assert.match(
      modernizationTask.reviewWorkflow.keyQuestionsAnswered.join('\n'),
