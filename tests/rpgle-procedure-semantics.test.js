...
`,
  ...
  try {
    ...
    assert.deepEqual(
      result.calls.map(entry => entry.name),
      ['INVPGM']
    );
    assert.deepEqual(
      result.procedures.map(entry => entry.name),
      ['MAIN', 'LOCALHELPER']
    );
    assert.deepEqual(
      result.prototypes.map(entry => entry.name),
      ['PROCESSORDER']
    );

    assert.deepEqual(
      result.procedureCalls.map(entry => `${entry.ownerName}:${entry.name}:${entry.resolution}`),
      ['MAIN:<DYNAMIC>:DYNAMIC', 'MAIN:MISSINGPROC:UNRESOLVED', 'MAIN:PROCESSORDER:EXTERNAL']
    );
  } finally {
...
`,
  ...
  try {
    const result = scanRpgFile(sourceFile);
    assert.deepEqual(
      result.procedures.map(entry => `${entry.kind}:${entry.name}`),
      ['SUBROUTINE:INITSR']
    );
    assert.deepEqual(
      result.procedureCalls.map(
        entry => `${entry.ownerName}:${entry.name}:${entry.resolution}:${entry.targetKind}`
      ),
      ['LEGACYPGM:INITSR:INTERNAL:SUBROUTINE', 'LEGACYPGM:MISSING:UNRESOLVED:UNRESOLVED']
    );
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true });
  }
  ...
  try {
    ...
    assert.ok(
      canonicalAnalysis.relations.some(
        entry => entry.type === 'CALLS_PROCEDURE' && entry.attributes.resolution === 'EXTERNAL'
      )
    );
...
`,
  ...
  try {
    const result = scanRpgFile(sourceFile);
    assert.deepEqual(
      result.nativeFiles.map(
        entry => `${entry.name}:${entry.kind}:${entry.keyed}:${entry.declaredAccess.join('/')}`
      ),
      [
    ...
    assert.deepEqual(
      result.nativeFileAccesses.map(
        entry =>
          `${entry.ownerName}:${entry.fileName}:${entry.opcode}:${entry.accessKind}:${entry.recordFormat || '-'}`
      ),
      [
...
`,
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['INPUT:DISK:true:READ', 'QPRINT:PRINTER:false:WRITE', 'SCREEN:WORKSTN:false:']
    );
    assert.deepEqual(
      result.nativeFileAccesses.map(
        entry =>
          `${entry.fileName}:${entry.opcode}:${entry.accessKind}:${entry.recordFormat || '-'}`
      ),
      [
        'INPUT:CHAIN:READ:-',
        'INPUT:READ:READ:-',
...
`,
  ...
  try {
    ...
    assert.ok(
      canonicalAnalysis.relations.some(
        entry =>
          entry.type === 'ACCESSES_NATIVE_FILE' && entry.attributes.recordFormat === 'ORDERFMT'
      )
    );
