...
test('runAnalyzeCore does not auto-load local knowledge artifacts after reset', () => {
  ...
  fs.writeFileSync(path.join(localKnownFactsDir, 'dev.json'), JSON.stringify({
    schemaVersion: 1,
  ...
  try {
    ...
    assert.equal(
      coreResult.context.notes.some((note) => /PUI pattern|known facts|knowledge/i.test(String(note))),
      false,
    );
  } finally {
