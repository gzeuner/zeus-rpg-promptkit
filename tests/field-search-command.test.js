...
const { normalizeMode, parseMaxResultsOption } = require('../src/cli/commands/fieldSearchCommand');
...
test('normalizeMode accepts local/remote/xref/all only', () => {
  ...
  assert.throws(() => normalizeMode('invalid'), /--mode must be one of local, remote, xref, all/);
});
...
test('parseMaxResultsOption validates positive integer', () => {
  ...
  assert.throws(() => parseMaxResultsOption('0'), /--max-results/);
  assert.throws(() => parseMaxResultsOption('foo'), /--max-results/);
});
