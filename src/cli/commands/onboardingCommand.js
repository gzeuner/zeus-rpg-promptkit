...
async function runFetchOrAnalyze(profile) {
  ...
  const src = await question('Source directory (default ./rpg_sources): ') || './rpg_sources';
  const prog = await question('Program to analyze (e.g. ORDERPGM): ');
...
async function runOnboarding(args = {}) {
  ...
  try {
    ...
    if (!await confirm(rl, 'Start the onboarding wizard? (Y/n) ')) {
      console.log('Aborted.');
      return;
    }

    const profilesPath = await ensureProfiles(rl);
    const profile = await selectProfile(rl, profilesPath);

    await runDoctor(profile);
    await discoverSource(profile);
    await objectAndTableDiscovery(profile);
    await runFetchOrAnalyze(profile);
