...
function printDbRuntimeConflictWarnings(dbConfig, { writeLine = line => console.warn(line) } = {}) {
  ...
  writeLine(
    '[WARN] Runtime config mismatch detected: environment overrides the selected profile target.'
  );
...
function buildEnvProfileConflictMessage({ profile, path, profileValue, envVar, effectiveValue }) {
  ...
  const profileText = profileValueLabel ? `"${profileValueLabel}"` : 'a configured target';
  const effectiveText = effectiveValueLabel ? `"${effectiveValueLabel}"` : 'a runtime override';
...
function buildDbRuntimeConflictDiagnostics(dbConfig, { profile = '' } = {}) {
  const warnings = getDbRuntimeConflictWarnings(dbConfig);
  const seen = new Set();
  const diagnostics = [];
