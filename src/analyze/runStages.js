...
function normalizeStageDefinition(stage) {
  ...
  const registrationOrder = Number.isInteger(stage.registrationOrder)
    ? stage.registrationOrder
    : null;
  const before = Array.isArray(stage.before)
    ? stage.before.map(entry => String(entry).trim()).filter(Boolean)
    : [];
  const after = Array.isArray(stage.after)
    ? stage.after.map(entry => String(entry).trim()).filter(Boolean)
    : [];
...
function buildStageLifecycleHandlers(stage, lifecycleHooks) {
  const beforeHandlers = lifecycleHooks.map(entry => entry.beforeStage).filter(Boolean);
  const afterHandlers = lifecycleHooks.map(entry => entry.afterStage).filter(Boolean);
  const errorHandlers = lifecycleHooks.map(entry => entry.onStageError).filter(Boolean);
...
function runStages(stages, initialState, options = {}) {
  ...
  return stages.reduce(
    (state, stage) => {
      ...
      const { beforeHandlers, afterHandlers, errorHandlers } = buildStageLifecycleHandlers(
        stage,
        lifecycleHooks
      );
    ...
    {
      ...initialState,
      stageReports: Array.isArray(initialState && initialState.stageReports)
        ? initialState.stageReports
        : [],
      diagnostics: Array.isArray(initialState && initialState.diagnostics)
