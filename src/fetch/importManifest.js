...
function createNormalizationPolicy(policy = null) {
  ...
  return {
    ...
    checksumAlgorithm: String(
      source.checksumAlgorithm || DEFAULT_NORMALIZATION_POLICY.checksumAlgorithm
    ),
  };
}
...
function getImportManifestEntryOrigin(entry) {
  const origin =
    entry && entry.origin && typeof entry.origin === 'object' ? entry.origin : entry || {};
  ...
  return {
    ...
    memberPath:
      origin.memberPath || buildMemberPath(origin.sourceLib, origin.sourceFile, origin.member),
...
function getImportManifestEntryExport(entry, importManifest = null) {
  const exportRecord =
    entry && entry.export && typeof entry.export === 'object' ? entry.export : entry || {};
  return {
    status: exportRecord.status || (entry && entry.exported === false ? 'failed' : 'exported'),
    transportRequested:
      exportRecord.transportRequested ||
      (importManifest && importManifest.transportRequested) ||
      null,
    transportUsed:
      exportRecord.transportUsed ||
      (entry && entry.transportUsed) ||
      (importManifest && importManifest.transportUsed) ||
      null,
    fallbackUsed: Boolean(exportRecord.fallbackUsed || (entry && entry.fallbackUsed)),
    command: exportRecord.command || (entry && entry.command) || '',
    streamFileCcsid:
      Number(
        exportRecord.streamFileCcsid ||
          (entry && entry.streamFileCcsid) ||
          (importManifest && importManifest.streamFileCcsid)
      ) || null,
    encodingPolicy:
      exportRecord.encodingPolicy ||
      (entry && entry.encodingPolicy) ||
      (importManifest && importManifest.encodingPolicy) ||
      null,
    ...
    messages: Array.isArray(exportRecord.messages)
      ? exportRecord.messages
      : Array.isArray(entry && entry.messages)
        ? entry.messages
        : [],
...
function getImportManifestEntryValidation(entry) {
  const validation =
    entry && entry.validation && typeof entry.validation === 'object'
      ? entry.validation
      : entry || {};
...
function summarizeImportManifest(importManifest, options = {}) {
  ...
  const exportedFileCount = importManifest.files.filter(
    entry => getImportManifestEntryExport(entry, importManifest).status === 'exported'
  ).length;
  ...
  return {
    ...
    sourceLib:
      importManifest.remote && importManifest.remote.sourceLib
        ? normalizeName(importManifest.remote.sourceLib)
        : null,
    ...
    fileCount:
      importManifest.summary && Number.isFinite(Number(importManifest.summary.fileCount))
        ? Number(importManifest.summary.fileCount)
        : importManifest.files.length,
    exportedFileCount:
      importManifest.summary && Number.isFinite(Number(importManifest.summary.exportedFileCount))
        ? Number(importManifest.summary.exportedFileCount)
        : exportedFileCount,
    failedFileCount:
      importManifest.summary && Number.isFinite(Number(importManifest.summary.failedFileCount))
        ? Number(importManifest.summary.failedFileCount)
        : failedFileCount,
    invalidFileCount:
      importManifest.summary && Number.isFinite(Number(importManifest.summary.invalidFileCount))
        ? Number(importManifest.summary.invalidFileCount)
        : 0,
    warningCount:
      importManifest.summary && Number.isFinite(Number(importManifest.summary.warningCount))
...
function buildImportManifest({ options, summary, exportRecords, localDestination }) {
  const normalizationPolicy = createNormalizationPolicy();
  const validation = validateSourceFiles(
    exportRecords.map(record => record.localPath).filter(Boolean),
    {
  ...
  return {
    ...
    request: {
      sourceFiles: Array.isArray(options.files)
        ? options.files.map(value => normalizeName(value)).filter(Boolean)
        : [],
      members: Array.isArray(options.members)
        ? options.members.map(value => normalizeName(value)).filter(Boolean)
        : [],
      replace: options.replace !== false,
    },
    localDestination,
    transportRequested: String(options.transport || '')
      .trim()
      .toLowerCase(),
    transportUsed: summary.transportUsed || null,
    streamFileCcsid: Number(options.streamFileCcsid) || null,
    ...
    summary: {
      ...
      exportedFailed: Math.max(
        0,
        Number(summary.exportedTotal || 0) - Number(summary.exportedSuccess || 0)
      ),
